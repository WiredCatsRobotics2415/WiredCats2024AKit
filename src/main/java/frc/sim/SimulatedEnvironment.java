...
public class SimulatedEnvironment {
    @Getter
    private double[] currentNoteTargetInfo;
        ...
        for (Note note : NoteVisualizer.getCurrentNotes()) {
            ...
            if (distance < maxNoteVisibleDistance) {
                double angleRad = Math.atan2(
                        noteTranslation.getY() - robotPosition.getY(), noteTranslation.getX() - robotPosition.getX());
    ...
    public void resetField(boolean includePreload) {
        ...
        if (includePreload)
            RobotVisualizer.takeNote(
                    new Note(new Translation3d(0, 0, 0))); // Can be zeros bc it will be updated by robotvisualizer
    }
}
